...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    if (nPrevHeight < 1) {
        return 4500000 * COIN;
    } else if (nPrevHeight >= 1 && nPrevHeight < 50) {
        return 2 * COIN;
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
    const CChainParams& chainparams = ::Params();
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (nSpendHeight >= chainparams.GetConsensus().nAnodosHeight && IsInputBanned(tx.vin[i], inputs))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ANDS): transaction %s conflicts with transaction lock %s",
    ...
    if(!founderTransaction) {
		...
		return state.DoS(0, error("CheckBlock(ANDS): transaction %s does not contains founder transaction",
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        int protonChangeBlock = LEVEL_COLLATERAL_START_HEIGHT + 10000;
        int height = chainActive.Height();
